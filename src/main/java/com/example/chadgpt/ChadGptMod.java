...
public class ChadGptMod {
    ...
    // Background workers for HTTP; never stall the server thread.
    ...
    // Rolling chat buffer; oldest at the head.
    ...
    // Endpoints and auth.
    private static final String API_URL = System.getProperty("chadgpt.api", "https://api.openai.com/v1/chat/completions");
    ...
    // Vector store id for the Silent Gear assister path.
    ...
    private static final int MAX_SAY_CHARS = Integer.parseInt(System.getProperty("chadgpt.max_chars", "500"));
    ...
    private static final int CHAT_CONNECT_MS = Integer.parseInt(System.getProperty("chadgpt.chat_connect_ms", "20000"));
    private static final int CHAT_READ_MS    = Integer.parseInt(System.getProperty("chadgpt.chat_read_ms",    "90000"));
    ...
    public void onChat(ServerChatEvent event) {
        ...
        // Record every line; oldest evicted when over cap.
        ...
        if (hasSilent) {
            // Silent Gear assister; Responses API with your vector store; request body uses only your specified keys.
            POOL.submit(() -> {
                String line = fetchFromResponsesWithFileSearch(context, latestUserMessage);
            ...
            // Regular ChadGPT; Chat Completions path; explicit non-stream; chat stays visible.
            POOL.submit(() -> {
                String line = fetchOneLine(context, latestUserMessage);
    ...
    // Regular ChadGPT; Chat Completions; non-stream.
    private String fetchOneLine(List<ChatLine> previous, String latestUserMessage) {
        ...
        try {
            ...
            body.addProperty("stream", false); // explicit; non-streamed
            ...
            // Prior lines; oldest first.
            for (ChatLine c : previous) {
                JsonObject prev = new JsonObject();
                prev.addProperty("role", "user");
                prev.addProperty("content", c.author + ": " + c.text);
                messages.add(prev);
            }

            JsonObject usr = new JsonObject();
            usr.addProperty("role", "user");
            usr.addProperty("content", latestUserMessage);
            messages.add(usr);
            ...
            for (int attempt = 0; attempt <= HTTP_RETRIES; attempt++) {
                ...
                try {
                    ...
                    conn.setConnectTimeout(CHAT_CONNECT_MS);
                    conn.setReadTimeout(CHAT_READ_MS);
                    conn.setDoOutput(true);
                    conn.setRequestMethod("POST");
                    conn.setRequestProperty("Content-Type", "application/json");
                    conn.setRequestProperty("Authorization", "Bearer " + apiKey);
                    conn.setRequestProperty("User-Agent", "ChadGPT-Forge/1.4");
                    ...
                    LOG.warn("Chat API attempt {} failed: {}", attempt + 1, e.toString());
                } finally {
            ...
            if (code / 100 != 2 || resp == null) {
                LOG.warn("OpenAI Chat API error {}; giving up", code);
            ...
            LOG.warn("OpenAI call failed", ex);
            return "Network gremlins; try again soon.";
        }
    }

    // Silent Gear assister; Responses API with file_search over your vector store.
    // Request body includes only: model; instructions; input; tools.
    private String fetchFromResponsesWithFileSearch(List<ChatLine> previous, String latestUserMessage) {
        String apiKey = System.getenv(API_KEY_ENV);
        if (apiKey == null || apiKey.isEmpty()) {
            return "Set the " + API_KEY_ENV + " environment variable for ChadGPT.";
        }
        ...
        String instructions =
                "You are ChadGPT; a minecraft player assisting other players on a server. " +
                "The players in this minecraft server do not have access to your knowledgebase files or other in-game files. Do not expose the existence of those files. " +
        ...
        // Build input string with context then the latest message; value shape is up to us.
        ...
        try {
            ...
            // Responses API parsing. Prefer output[..].content[..].text. Fall back to output_text.
